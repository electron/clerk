...
2. Any instruction apps must act on ("use X instead", "enable Y to keep Z"), in the same sentence as the change it belongs to, after a semicolon. Only when a ${BREAKING_LABEL} change and its instruction cannot fit in one sentence, use two bullets: the change with its conditions, then a full sentence saying what apps must now do, naming the API it applies to.
...
5. The names developers search for: public APIs, events, options, CLI flags, fuses and tools. Keep every one of them unless the sentence cannot fit otherwise; shorten other words first. Keep each name exactly as written: a shorter form is a different API (\`module.builtinModules\` is not \`require('module').builtinModules\`). Never argue that one name covers another unless the note says so. Replace a list of names with a group name only when the group is exactly the listed names and developers would recognise it.
...
3. It keeps the public API, event, option, CLI flag, fuse and tool names from the note. Each candidate lists the backticked names it leaves out; each of those is a problem unless it is internal, or an exact group name that developers would recognise replaces it, or it is mentioned only in passing and keeping it could not fit even with bullets. A claim that one name covers another is not a reason unless the note says so, and a shortened or rewritten form of a name (\`module.builtinModules\` for \`require('module').builtinModules\`) counts as leaving it out even when the reasons call the two equivalent.
