...
5. The names developers search for: public APIs, events, options, CLI flags, fuses and tools. Keep every one of them unless the sentence cannot fit otherwise; shorten other words first. Never argue that one name covers another unless the note says so. Replace a list of names with a group name only when the group is exactly the listed names and developers would recognise it.
...
3. It keeps the public API, event, option, CLI flag, fuse and tool names from the note. Each candidate lists the backticked names it leaves out; each of those is a problem unless it is internal, or an exact group name that developers would recognise replaces it, or it is mentioned only in passing and keeping it could not fit even with bullets. A claim that one name covers another is not a reason unless the note says so.
